void main() {
    int count = 0;
    String toguess = "apfel";                           //zu errratendes Wort
...
void lettercheck(String right, String guess){           //kontrolliert jeden buchstaben und gibt ihn in der entsprechenden Farbe zurück
    for (int i = 0; i < 5; i++){
        ...
        if (right.contains(a)){
            for (int j = 0; j < 5; j++){
                if (guess.charAt(i) == right.charAt(j)){
                    if (i == j){
                ...
                if (guess.charAt(i) == right.charAt(j)) {
                    if (i != j){
                        IO.print(Backgroundcolor.ANSI_YELLOW+guess.charAt(i)+Backgroundcolor.ANSI_RESET);
